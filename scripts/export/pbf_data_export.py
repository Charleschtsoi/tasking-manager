...
Frontend download path contract
-------------------------------
Tasking Manager's DownloadOsmData UI fetches:

  {EXPORT_TOOL_S3_URL}/TM/hotosm_project_{id}/{category}/{geom_type}/
      hotosm_project_{id}_{category}_{geom_type}_{format}.zip

where ``category`` is lowercase (roads, buildings, waterways, landuse).
...
sandbox : bool
    ...
    projects; False returns only non-sandbox projects.
...
def _import_duckdb():
    """Lazy import so path helpers can be unit-tested without DuckDB installed."""
    import duckdb
...
# Category display keys may be Title Case; S3 path segments are always lowercased
# to match frontend DownloadOsmData (roads/buildings/waterways/landuse).
# --------------------------------------------------------------------------- #
DEFAULT_CONFIG = {
    "dataset": {
...
def category_slug(category_name: str) -> str:
    """Normalize category names to the lowercase slug used in TM S3 keys / UI."""
    return category_name.strip().lower()


def build_object_key(rel_key: str, output_key_prefix: str = "") -> str:
    """Join an optional top-level prefix with a relative export path for S3."""
    rel = rel_key.replace(os.sep, "/").lstrip("/")
    prefix = (output_key_prefix or "").strip().strip("/")
    if not prefix:
        return rel
    return f"{prefix}/{rel}"
...
class Extractor:
    ...
    def export_project(self, project_id, boundary_geojson, dataset, categories):
        ...
        for cat in categories:
            ...
            # Frontend DownloadOsmData uses lowercase category path segments.
            cname_slug = category_slug(cname)
            for ftype in body["types"]:
                ...
                for fmt in body["formats"]:
                    ...
                    base = f"{prefix}_{cname_slug}_{ftype}_{fmt}"
                    rel_dir = os.path.join(folder, prefix, cname_slug, ftype)
                    abs_dir = os.path.join(self.work_dir, rel_dir)
                    os.makedirs(abs_dir, exist_ok=True)
                    # Isolate each format write so multi-format zips do not
                    # accidentally include sibling .geojson/.shp/.kml outputs.
                    fmt_dir = os.path.join(abs_dir, f"_build_{fmt}")
                    if os.path.isdir(fmt_dir):
                        shutil.rmtree(fmt_dir, ignore_errors=True)
                    os.makedirs(fmt_dir, exist_ok=True)
                    out_file = os.path.join(fmt_dir, f"{base}.{fmt}")
                    try:
                        self.con.execute(
                    ...
                    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                        for fn in sorted(os.listdir(fmt_dir)):
                            zf.write(os.path.join(fmt_dir, fn), arcname=fn)
                    shutil.rmtree(fmt_dir, ignore_errors=True)
